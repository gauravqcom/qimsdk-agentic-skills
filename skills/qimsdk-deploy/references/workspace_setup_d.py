...
Contrast with Mode C (gstreamer-app-builder C sample apps):
  ...
  - Mode D builds each app OUT OF TREE as a standalone SDK consumer, in its own
    ...
    it from the SDK target sysroot's standard lib search path (no
    find_library() needed). Nothing shared is mutated.
...
libqimsdk-app-builder.so ships inside the SDK target sysroot — there is NO
SDK-from-source build step (contrast Mode C's git clone + full cmake of the
plugin tree).

State transitions (see workspace_state.detect_mode_d_state):
  0 → 1: Install Yocto SDK (fetch zip → unzip → run .sh installer)
  1 → 2: Push main.cc + the artifact's own CMakeLists.txt to per-app dir
...
#
# The artifact's own CMakeLists.txt (generated by qimsdk-cpp-app-builder) links
# `qimsdk-app-builder` via a plain target_link_libraries() — no find_library().
# Verified: cmake resolves this fine against the SDK target sysroot's standard
# lib search path when environment-setup-*-qcom-linux is sourced (CXX has
# --sysroot baked in), so the artifact's CMakeLists.txt is pushed AS-IS. No
# wrapper is generated. install()'s ${QIMSDK_BINDIR} is unused/undefined here
# since only `cmake --build` (not `--target install`) is run — harmless.
...
def _push_source_files(ssh_dc, artifact_path, remote_app_dir, target_name):
    """
    Push main.cc and CMakeLists.txt (both from the artifact, unmodified) to
    remote_app_dir on the workstation. Normalizes CRLF, verifies after push.

    The artifact's own CMakeLists.txt IS used for the build — verified against
    a real Yocto SDK: `target_link_libraries(${TEST_TARGET} PRIVATE
    qimsdk-app-builder)` resolves via cmake's standard lib search path once
    environment-setup-*-qcom-linux is sourced (CXX has --sysroot baked in), no
    find_library()/wrapper needed.
    ...
    if not art_cmake.exists():
        return False, 'Artifact missing required file: CMakeLists.txt'
    ...
    # Push the artifact's own CMakeLists.txt (CRLF-normalized), unmodified.
    cmake_bytes = art_cmake.read_bytes().replace(b'\r\n', b'\n').replace(b'\r', b'\n')
    remote_cmake = f'{remote_app_dir}/CMakeLists.txt'
    try:
        ssh_dc.push_bytes(cmake_bytes, remote_cmake)
    except Exception as e:
        return False, f'Failed to push CMakeLists.txt to workstation: {e}'
